...
def main():
    ...
    if st.session_state.modelo_entrenado:
        ...
        with col1:
            st.metric('Error Medio (unidades)', f"{st.session_state.error_analysis['error_medio_unidades']:.2f}")
            st.metric('Error Máximo (unidades)', f"{st.session_state.error_analysis['maximo_error_unidades']:.2f}")
        with col2:
            st.metric('Stock Insuficiente (%)', f"{st.session_state.error_analysis['stock_insuficiente']:.1f}%")
            st.metric('Error Mediano (unidades)', f"{st.session_state.error_analysis['error_mediano_unidades']:.2f}")
